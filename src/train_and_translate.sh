...
translate=true
# ^-- if false, does not translate dev_raw_src

tuning_lines=4000
# ^-- takes this many random lines from train for tuning
...
dev_raw_src=$shared/data/dev.raw.$lang_src
# ^-- translates this
out_dev_translated=$shared/data/dev.translated.$lang_dst
# ^-- translates here
...
if [ "$tokenize" = true ] ; then
    ...
    inp=$dev_raw_src out=$data_path/dev.tok.$lang_src lang=$lang_src preprocess
